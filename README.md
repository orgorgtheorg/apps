...
- **Artwork is convention, not manifest fields**: `icon.png` (512×512, square — corners are rounded by the UI) and `screenshots/01.png…` are detected by the sync and served as raw URLs pinned at the synced commit. No files = the store renders a generated placeholder tile. `scripts/make-icons.py` draws the catalog's house-style icon for an app from its `accent` colour and a per-app mark, so the set stays consistent (`python3 scripts/make-icons.py <id>`; needs Pillow).
